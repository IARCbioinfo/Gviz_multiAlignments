...
if(is.null(args$sample_names)) {
  args$sample_names="FILE"
} else if(args$sample_names=="SAMPLE"){
...
get_htTrack <- function(bams,bam_folder,plot_grtracks,UCSC,sTrack,grtrack,chr,pos,paired=TRUE){
  l=list()
  ...
  if(UCSC & plot_grtracks){
    ht <- HighlightTrack(trackList = c(l,sTrack, grtrack), start = c(pos), width =0,chromosome = chr)
...
plotGviz <- function(sTrack,ref_genome,txdb,annotation,UCSC,chr,pos,bams,bam_folder,w=50,w_zoomout=1000,paired=TRUE,nb_toplot=5){
  ...
  if(UCSC & plot_grtracks){
    res=get_htTrack(bams,bam_folder,plot_grtracks,UCSC,sTrack,grtrack,chr,pos,paired)
    grid.newpage()
